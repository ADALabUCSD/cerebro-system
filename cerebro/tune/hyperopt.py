...
def _validate_and_generate_hyperopt_search_space(search_space):
    ...
    for k in search_space:
        if isinstance(search_space[k], _HPChoice):
            ...
            hyperopt_space[k] = hp.loguniform(k, np.log(np.power(10, search_space[k].min)), np.log(np.power(
                10, search_space[k].max)))
        elif isinstance(search_space[k], _HPQLogUnifrom):
            hyperopt_space[k] = hp.qloguniform(k, np.log(np.power(10, search_space[k].min)), np.log(np.power(
                10, search_space[k].max)), search_space[k].q)
        else:
